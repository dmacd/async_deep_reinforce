...
def restore_checkpoint(sess, saver, checkpoint_name='checkpoint', path=constants.CHECKPOINT_DIR):
    """
    ...
    if checkpoint and checkpoint.model_checkpoint_path:
      ...
      # cleanup: now done by caller
      # tokens = checkpoint.model_checkpoint_path.split("-")
      # # set global step
      # global_t = int(tokens[1])
      # print ">>> global step set: ", global_t

      # restore counter-> global step var
...
def save_checkpoint(sess, saver, global_step, checkpoint_name='checkpoint', path=constants.CHECKPOINT_DIR):
